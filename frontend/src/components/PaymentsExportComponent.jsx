...
/**
 * PaymentsExportComponent
 * Enterprise UI Component for the payments module.
 */
const PaymentsExportComponent = ({ initialData, userRole, theme, onUpdate, isReadOnly, configId }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const endpoint = `/api/v1/payments/export/?page=${page}&search=${searchQuery}&sort=${sortField}&${sortDirection}`;
    ...
    const handleFormSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            await axios.post(`/api/v1/payments/export/`, formData);
    ...
    const deleteSelected = async () => {
        ...
        try {
            await Promise.all(Array.from(selectedRows).map(id => 
                axios.delete(`/api/v1/payments/export/${id}/`)
            ));
    ...
    if (error && !data.length) {
        return (
            <Alert variant="danger" className="m-4 shadow-sm">
                <Alert.Heading>Error Loading PaymentsExportComponent</Alert.Heading>
                <p>{error}</p>
        ...
        <Container fluid className={`py-4 ${isDarkTheme ? "bg-dark text-light" : "bg-light text-dark"}`}>
            <Card className="shadow-lg border-0 rounded-lg">
                <Card.Header className="bg-primary text-white d-flex justify-content-between align-items-center p-4">
                    <h4 className="mb-0 m-0">PaymentsExportComponent Dashboard</h4>
                    {canEdit && (
                        <div>
            ...
            <Modal show={isModalOpen} onHide={() => setIsModalOpen(false)} size="lg" backdrop="static">
                <Modal.Header closeButton className="bg-light"><Modal.Title>Create New PaymentsExportComponent</Modal.Title></Modal.Header>
...
PaymentsExportComponent.propTypes = {
...
PaymentsExportComponent.defaultProps = {
    initialData: [],
    userRole: "guest",
    theme: "light",
    isReadOnly: false,
...
export default PaymentsExportComponent;
